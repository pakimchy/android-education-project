...
public class PinnedHeaderListView extends ListView implements IIndexBarFilter {
    ...
    public void setAdapter(ListAdapter adapter) {
    	if (adapter instanceof IPinnedHeader) {
    		this.mAdapter = (IPinnedHeader)adapter;
    	}
        if (mAdapter != null) {
        	View headerView = mAdapter.getPinnedHeaderView();
        	...
        	IndexBarView ibv = mAdapter.getIndexBarView();
        	lp = (LayoutParams)ibv.getLayoutParams();
        	if (lp == null) {        		
        		lp = new LayoutParams(getContext().getResources().getDimensionPixelSize(R.dimen.index_bar_view_width),LayoutParams.MATCH_PARENT);
        		ibv.setLayoutParams(lp);
        	}
        	if (ibv != null) {
        		ibv.setPinnedHeaderListView(this);
        		setIndexBarView(ibv);
        	}
        	View preview = mAdapter.getPreviewView();
        	lp = (LayoutParams)preview.getLayoutParams();
        	if (lp == null) {
